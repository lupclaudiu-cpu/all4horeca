    ...
    <div className="fixed inset-0 z-[90] overflow-y-auto bg-black/60 p-3 backdrop-blur-sm sm:p-6">
      <div className="mx-auto max-w-4xl rounded-[2rem] bg-white p-5 shadow-2xl sm:p-7">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase text-[#2563eb]">Variante și extra</p>
            <h2 className="text-2xl font-black">{product.name}</h2>
          </div>
          <button type="button" onClick={onClose} className="text-xs font-black text-[#64748b]">Închide</button>
        </div>
        <button type="button" onClick={() => setEditing("new")} className="mt-5 rounded-xl bg-[#2563eb] px-4 py-3 text-xs font-black text-white">Creează grup</button>
        {error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-xs font-bold text-red-700">{error}</p>}
        <div className="mt-5 space-y-3">
          {loading ? <div className="h-32 animate-pulse rounded-2xl bg-[#f1f5f9]" /> : groups.map((group) => (
            <article key={group.id} className="rounded-2xl border border-[#e2e8f0] p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => void setProductOptionGroupActive(group.id, !group.active).then(load).then(onChanged).catch((reason) => setError(String(reason)))} className="rounded-xl bg-[#f1f5f9] px-3 py-2 text-[11px] font-black">{group.active ? "Dezactivează" : "Activează"}</button>
                  <button type="button" onClick={() => setEditing(group)} className="rounded-xl bg-[#0f172a] px-3 py-2 text-[11px] font-black text-white">Modifică</button>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {group.options.map((option) => <span key={option.id} className="rounded-full bg-[#eff6ff] px-3 py-2 text-[10px] font-black">{option.name} {option.priceDelta ? `+${formatPrice(option.priceDelta)}` : ""}{option.multiplyByProductQuantity ? " × produs" : ""}</span>)}
              </div>
            </article>
          ))}
        </div>
...
function GroupForm({ productId, group, onClose, onSaved }: { productId: string; group?: ProductOptionGroup; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const addOption = () => setData((current) => ({ ...current, options: [...current.options, { id: crypto.randomUUID(), name: "", priceDelta: 0, multiplyByProductQuantity: false, active: true, sortOrder: current.options.length }] }));
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 p-4">
      <form onSubmit={submit} className="mx-auto max-w-2xl rounded-[2rem] bg-white p-6">
        ...
        <div className="mt-6 flex items-center justify-between"><p className="text-xs font-black uppercase text-[#2563eb]">Opțiuni</p><button type="button" onClick={addOption} className="rounded-xl bg-[#f1f5f9] px-3 py-2 text-xs font-black">Adaugă opțiune</button></div>
        <div className="mt-3 space-y-2">
          {data.options.map((option, index) => (
            <div key={option.id} className="grid gap-2 rounded-xl border border-[#e2e8f0] p-3 sm:grid-cols-[1fr_120px_90px_auto]">
              ...
              <input type="number" step=".01" value={option.priceDelta} onChange={(event) => setData({ ...data, options: data.options.map((item, itemIndex) => itemIndex === index ? { ...item, priceDelta: Number(event.target.value) } : item) })} className={compactClass} />
              ...
              <label className="flex items-center gap-2 text-[11px] font-bold sm:col-span-4">
                <input
                  type="checkbox"
                  checked={option.multiplyByProductQuantity}
                  onChange={(event) => setData({ ...data, options: data.options.map((item, itemIndex) => itemIndex === index ? { ...item, multiplyByProductQuantity: event.target.checked } : item) })}
                  className="accent-[#2563eb]"
                />
                Multiplică această opțiune cu numărul produselor principale
              </label>
            </div>
          ))}
        </div>
        <button disabled={saving || !data.name || data.options.some((option) => !option.name)} className="mt-6 w-full rounded-2xl bg-[#2563eb] px-5 py-4 text-sm font-black text-white disabled:opacity-50">{saving ? "Se salvează..." : "Salvează grupul"}</button>
      </form>
    </div>
  );
}

const inputClass = "mt-2 w-full rounded-xl border border-[#cbd5e1] px-4 py-3 text-sm";
const compactClass = "min-w-0 rounded-lg border border-[#cbd5e1] px-3 py-2 text-xs";
...
function Check({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) { return <label className="flex items-center gap-2 text-xs font-black"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="accent-[#2563eb]" />{label}</label>; }
