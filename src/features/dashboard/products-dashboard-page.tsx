...
export function ProductsDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="rounded-xl bg-white px-4 py-3 text-xs font-black shadow-sm"
            >
      ...
      <div className="mt-6 overflow-hidden rounded-[1.75rem] border border-black/5 bg-white shadow-[0_10px_30px_rgba(24,18,12,0.04)]">
        {loading ? (
          <div className="h-80 animate-pulse bg-[#f8f5f2]" />
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="relative size-16 shrink-0 overflow-hidden rounded-xl bg-[#fff6ec]">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 truncate text-[11px] text-[#8b8580]">
                  ...
                  <div className="flex flex-wrap gap-2 md:justify-end">
                    <button
                      ...
                      className="rounded-xl bg-[#fff4ed] px-3 py-2 text-[11px] font-black text-[#d9470d]"
                    >
                      Variante & Extra
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="rounded-xl bg-[#f3f0ed] px-3 py-2 text-[11px] font-black"
                    >
                    ...
                    <button
                      ...
                      className="rounded-xl bg-[#171411] px-3 py-2 text-[11px] font-black text-white"
                    >
                      Modifică
                    </button>
                  </div>
    ...
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-black/60 p-3 backdrop-blur-sm sm:p-6">
      <form onSubmit={submit} className="mx-auto max-w-3xl rounded-[2rem] bg-white p-5 shadow-2xl sm:p-7">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <ProductField label="Preț" value={String(data.price)} onChange={(value) => update("price", Number(value))} type="number" />
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5">
          ...
          <ProductCheck label="Recomandat" checked={data.featured} onChange={(value) => update("featured", value)} />
        ...
        <div className="mt-6">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#ff5a1f]">Se comandă frecvent împreună</p>
        ...
        <button disabled={saving || uploading || !data.name || !data.categoryId} className="mt-6 w-full rounded-2xl bg-[#ff5a1f] px-5 py-4 text-sm font-black text-white disabled:opacity-50">
...
const inputClass = "mt-2 w-full rounded-xl border border-[#e6e0db] bg-[#fcfaf8] px-4 py-3 text-sm outline-none focus:border-[#ff5a1f]";

function ProductField({ label, value, onChange, type = "text", placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string }) {
  return <label><span className="text-xs font-black">{label}</span><input type={type} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className={inputClass} /></label>;
}
...
function ProductCheck({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) {
  return <label className="flex items-center justify-between gap-2 rounded-xl border border-[#eee9e4] px-3 py-3 text-xs font-black"><span className="truncate">{label}</span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 accent-[#ff5a1f]" /></label>;
}
...
function SmallBadge({ text }: { text: string }) {
  return <span className="rounded-full bg-[#fff1e9] px-2 py-1 text-[8px] font-black uppercase text-[#d9470d]">{text}</span>;
}
